...
.PHONY: help setup check check-strict scaffold scaffold-bug test test-template test-plugin setup-lint lint-markdown verify-template status clean cleanup-template-specs apply-branch-protection
...
test-template:  ## Test OpenSpec machinery locally (bash, git, Ruby; no network)
	ruby tests/workflows.rb
	ruby tests/openspec_core_test.rb

test-plugin:  ## Test the Magnific plugin (bash, python3; no network or account)
	bash tests/plugin.sh
	python3 tests/magnific_hooks_test.py
...
verify-template: check-strict test-template test-plugin lint-markdown  ## Verify template contracts and Markdown, not all CI/security jobs
